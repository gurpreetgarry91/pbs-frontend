...
const uploadMedia = async (
  ...
  const res = await fetch(ENDPOINTS.MEDIA, { method: "POST", headers: { ...authHeader(token) }, body: fd });
...
const deleteMedia = async (id: number, token?: string | null) => {
  ...
  const res = await fetch(`${ENDPOINTS.MEDIA}/${id}`, { method: "DELETE", headers: headers2 });
